...
public:
	...
	enum ReplicateChoice {
		...
		ARIByClasses,			// idem, avec ARI par clusters le plus �lev�
		...
		PredictiveClustering, // mesure le compromis entre les classes induites par le clustering, et
							  // la compacite / separabilite des clusters (mode supervis� uniquement)
							  ReplicateChoiceAutomaticallyComputed,		// ARI si supervis�, sinon Distance
	...
	/** nbre de fois que l'apprentissage doit �tre execut�. Au terme de ces clustering, on ne retiendra que le meilleur resultat */
	...
	/** garder la liste des attributs utilis�s lors du clustering (reper�s a partir de leur label) */
	void AddAttributes(const KWClass* kwc);

	/** garder la liste des noms d'attributs natifs et recod�s, aux fins d'affichage dans les rapports */
	...
	/** ecriture des rapports : obtenir un nom d'attribut natif, a partir de son nom recod� */
	ALString GetNativeAttributeName(const ALString recodedAttributeName) const;

	/** liste des attributs servant au calcul K-Means : cl� = nom, valeur = index de chargement de l'attribut */
	const ObjectDictionary& GetKMAttributeNames() const;

	/** liste des attributs : cl� = nom, valeur = indice permettant de retrouver l'index de chargement de l'attribut, dans la structure correspondante */
	const ObjectDictionary& GetLoadedAttributesNames() const;

	/** cl� = nom attribut recod�, valeur = nom attribut natif */
	...
	/** determine si l'objet pass� en parametre a une valeur manquante parmi ses attributs apr�s recodage */
	bool HasMissingKMeanValue(const KWObject*) const;

	/** determine si l'objet pass� en parametre a une valeur manquante parmi ses attributs natifs */
	...
	/** recuperer une chaine pourvue d'un suffixe num�rique, en evitant les doublons eventuels */
...
protected:
	...
	/** cl� = nom, valeur = IntegerObject * : poste concerne de l'attribut K-Means, dans la structure livLoadedAttributesLoadIndexes */
	ObjectDictionary odKMeanAttributesNames;

	/** cl� = nom d'attribut, valeur = IntegerObject * : poste concerne de l'attribut (K-Means ou non), dans la structure livLoadedAttributesLoadIndexes */
	ObjectDictionary odLoadedAttributesNames;

	/** cl� = nom attribut recod�, valeur = StringObject *, contenant le nom de l'attribut natif */
...
inline bool KMParameters::HasMissingKMeanValue(const KWObject* o) const {
	...
	// controle des valeurs manquantes, parmi les attributs servant � calculer le K-Means
	// (ce sont des continuous, en principe recod�s, sauf si on a choisi de ne pas faire de pretraitement)
