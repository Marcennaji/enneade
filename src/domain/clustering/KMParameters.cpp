...
void KMParameters::AddAttributes(const KWClass* kwc) {
	...
	// memoriser les attributs charg�s, � partir d'un dico
	...
	for (int attr = 0; attr < kwc->GetLoadedAttributeNumber(); attr++) {
		...
		odLoadedAttributesNames.SetAt(attribute->GetName(), io);// liste de tous les attributs charg�s
...
void KMParameters::AddRecodedAttribute(const KWAttribute* nativeAttribute, const KWAttribute* recodedAttribute) {
	...
	// garder la trace de la correspondance entre attribut natif et attribut recod�, aux fins d'affichage dans les rapports
...
boolean KMParameters::Check()
{
	...
	// si pas de variable cible, passer automatiquement la selection des replicates sur le crit�re "distance"
