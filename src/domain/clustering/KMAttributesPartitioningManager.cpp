...
void KMAttributesPartitioningManager::AddValueGroups(KWDRValueGroups* kwdrGroups, const ALString attributeName, const int maxValuesToAdd, const bool supervisedMode) {
	...
	// elaborer les libelles de modalit�s group�es
	...
	// stocker les libelles de modalit�s NON group�es, s'il n'y a pas plus de 10 modalit�s en tout (sans compter la modalite '*') :
	...
	// ajout du libell� "unseen values", en g�rant la presence eventuelle d'une modalit� ayant deja cette valeur.
	// ne pas toucher a l'ordonnancement d'origine des modalit�s
...
void KMAttributesPartitioningManager::CopyFrom(const KMAttributesPartitioningManager* aSource)
{
	...
	// copier les ObjectArray de modalit�s group�es ou d'intervalles (auparavant, supprimer les donnees eventuellement existantes)
	POSITION position = odAttributesPartitions.GetStartPosition();
	ALString key;
	Object* oCurrent;
