...
{
	// reconstruire un resultat K-Means et les parametres correspondants, � partir du dictionnaire de modelisation.
	...
	if (parameters->GetWriteDetailedStatistics()) {
		// creer les attributs CellIndex, servant a produire les rapports de frequences de modalit�s
	...
	// les index de chargement des attributs utilis�s
	parameters->PrepareDeploymentClass(predictorClass);

	// creation cluster "unique" des donn�es, servant au calcul des stats globales
	kmModelingClustering->CreateGlobalCluster();

	// extraire les modalites/intervalles � partir du dico
	if (parameters->GetWriteDetailedStatistics())
...
bool KMTrainedPredictor::CreateClusters(KWClass* predictorClass, KMClustering* clustering) {
	...
	KMParameters* parameters = cast(KMParameters*, clustering->GetParameters());
...
void KMTrainedPredictor::ExtractPartitions(KWClass* kwc) {
	...
	while (attribute != NULL)
	{
		...
		if (attribute->GetConstMetaData()->IsKeyPresent(KMPredictor::CELL_INDEX_METADATA)) { // attribut temporaire, cr�� uniquement pour produire des stats detaillees sur les variables categorielles
...
void KMTrainedPredictor::ExtractRankNormalization(const KWAttribute* attribute) {
	...
	cast(KMParameters*, kmModelingClustering->GetParameters())->AddRecodedAttribute(nativeAttribute, attribute);
...
void KMTrainedPredictor::ExtractBasicGrouping(const KWAttribute* attribute) {
	...
	ALString nativeName = attribute->GetDerivationRule()->GetSecondOperand()->GetAttributeName();
	KWAttribute* nativeAttribute = predictorClass->LookupAttribute(nativeName);
	parameters = cast(KMParameters*, kmModelingClustering->GetParameters());
...
void KMTrainedPredictor::AddCellIndexAttributes(KWTrainedPredictor* trainedPredictor) {
	...
	// pour chaque attribut pr�par� de type datagrid, ajouter un attribut temporaire de type ValueIndex, necessaire pour produire
	// les rapports de statistiques detaill�es.
	...
	while (attribute != NULL) {
		...
		if (attribute->GetConstMetaData()->IsKeyPresent(KMPredictor::PREPARED_ATTRIBUTE_METADATA)) {
			...
			if (attribute->GetStructureName() != "DataGrid") {
				// dico trafiqu� manuellement avant une �valuation ?
