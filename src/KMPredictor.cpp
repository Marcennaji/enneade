...
void KMPredictor::PrepareLocalModelClassForMerging(KWClass* trainedLocalModelClass, ALString attributesPrefix) {
	...
	while (attribute != NULL)
	{
		...
		trainedLocalModelClass->RenameAttribute(attribute, attributesPrefix + attribute->GetName());
...
KWPredictor* KMPredictor::CreateLocalModelPredictorFromCluster(KMCluster* cluster, KWClass* localModelClass, KWDatabase* localModelDatabase) {
	...
	if (parameters->GetLocalModelType() == KMParameters::NB)
		...
		localModelPredictor = new KWPredictorSelectiveNaiveBayes;
#ifdef _DEBUG
		((KWPredictorSelectiveNaiveBayes*)localModelPredictor)->GetSelectionParameters()->SetOptimizationAlgorithm("FFW"); // pendant tests uniquement, pour gain de vitesse d'execution
		cout << "DEBUG FFW" << endl;
#endif
	}
