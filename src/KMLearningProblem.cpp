...
KMLearningProblem::KMLearningProblem()
{
	...
	analysisResults->SetResultFilesPrefix("Enneade_");
...
void KMLearningProblem::ComputeStats()
{
		...
		else
		{
			...
			if (kmPredictor->GetLocalModelsPredictors().GetSize() > 0) {
				...
				for (int iLocalModel = 0; iLocalModel < localModelsPredictors.GetSize(); iLocalModel++) {
					...
					ALString sLocalModelPreparationReportName = BuildOutputFilePathName("cluster_" + cluster->GetLabel() + "_" + analysisResults->GetPreparationFileName());
					AddSimpleMessage("Writing preparation report for cluster " + cluster->GetLabel() + " local model : " + sLocalModelPreparationReportName);
					localModelPredictor->GetClassStats()->SetAllWriteOptions(false);
					localModelPredictor->GetClassStats()->SetWriteOptionStats1D(true);
					localModelPredictor->GetClassStats()->SetWriteOptionDetailedStats(true);
					localModelPredictor->GetClassStats()->WriteReportFile(sLocalModelPreparationReportName);
					localModelPredictor->GetClassStats()->SetAllWriteOptions(true);
					...
					const ALString sLocalModelsModelingReportName = BuildOutputFilePathName("cluster_" + cluster->GetLabel() + "_ModelingReport.xls");
	...
	if (bStatsOk and not TaskProgression::IsInterruptionRequested() and oaTrainedPredictors.GetSize() > 0)
	{
			...
			not GetTrainDatabase()->IsEmptySampling())
		{
			...
			localPredictorEvaluator.WriteEvaluationReport(BuildOutputFilePathName(analysisResults->GetTrainEvaluationFileName()),
			...
			not GetTrainDatabase()->IsEmptySampling())
		{
			...
			// Ecriture du rapport d'evaluation
			localPredictorEvaluator.WriteEvaluationReport(BuildOutputFilePathName(analysisResults->GetTestEvaluationFileName()),
	...
	if (trainedClassDomain.GetClassNumber() > 0)
	{
		...
		if (GetAnalysisResults()->GetModelingDictionaryFileName() != "" and not TaskProgression::IsInterruptionRequested())
		{
			...
			sModelingDictionaryFileName = BuildOutputFilePathName(GetAnalysisResults()->GetModelingDictionaryFileName());
