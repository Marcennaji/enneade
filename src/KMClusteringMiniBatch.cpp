	...
	if (bOk)
	{
		...
		while (not allInstances->IsEnd())
		{
			...
			if (kwoObject != NULL)
			{
				...
				if (targetAttribute != NULL)
					cluster->UpdateTargetProbs(oaTargetAttributeValues, targetAttribute, kwoObject); // mode supervisé : calculer la repartition des valeurs de l'attribut cible, dans chaque cluster
