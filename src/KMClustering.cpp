...
bool KMClustering::ComputeReplicate(ObjectArray* instances, const KWAttribute* targetAttribute)
{
	...
	if (instances->GetSize() == 0) {
		// ne pas faire un "assert", pour gérer correctement le cas ou une interruption de lecture de la base a eté demandée par l'utilisateur
	...
	// (uniquement dans le cas ou ces stats n'auraient pas deja été recuperees a partir d'un autre resultat)
	if (kmGlobalCluster == NULL)
		ComputeGlobalClusterStatistics(instances);

	if (kmGlobalCluster->GetFrequency() == 0) {
		// NB. ne pas utiliser GetCount(), car les instances ne sont pas gardées dans le cluster, seuls les stats et centroides sont gardés
	...
	if (not DoClusteringIterations(instances, instances->GetSize()))// iterations jusqu'à convergence
		return false;
	...
	if (recomputeCentroids)
		AddInstancesToClusters(instances);// affecte les instances aux clusters en fonction des centroides precédemment calculés, et met a jour les matrices de distances inter-clusters
	...
	if (targetAttribute != NULL) {
		...
		ComputeTrainingTargetProbs(targetAttribute); // mode supervisé : calculer la repartition des valeurs de l'attribut cible, dans chaque cluster
...
boolean KMClustering::DoClusteringIterations(const ObjectArray* instances, const longint maxInstances) {
	...
	assert(kmClusters->GetSize() > 0); // une premiere initialisation des clusters doit avoir été faite auparavant
	...
	while (continueClustering)
	{
		...
		if (parameters->GetMaxIterations() == -1) {
			continueClustering = false; // on se contente dans ce cas de l'initialisation des clusters qui a été réalisée, selon la methode choisie
		}
		else {
			// evaluer si le clustering doit se poursuivre
			// attention : minDistanceSum et epsilonIterations peuvent etre modifiés par l'appel
...
bool KMClustering::ManageConvergence(const int movements, const int iterationsDone,
	...
	if (movements > 0) {
		...
		if (fabs((distancesSum - newDistancesSum) / instancesCount) >= parameters->GetEpsilonValue() and
			...
			// --> si l'ecart entre la distance calculée a la precedente iteration et l'iteration courante est superieur
			...
			minDistanceSum = newDistancesSum; // Memorisation de la distance mini observée, toutes iterations confondues

			// Memorisation du modèle courant, car il est le meilleur
			CloneBestClusters();
		}
		else {
...
KMCluster* KMClustering::FindNearestClusterL1(KWObject* instance) {
	...
	// recuperer le cluster auquel appartient actuellement cette instance (NB. : en cours  de première initialisation des clusters, il n'y en a pas encore)
	...
	if (firstClusterToCheck == NULL) {
		...
		// le cluster de l'instance et son cluster le plus proche. En fonction du résultat, on pourra se passer de calculer la distance pour
		// les autres clusters
...
KMCluster* KMClustering::FindNearestClusterL2(KWObject* instance) {
	...
	if (firstClusterToCheck == NULL) {
		...
		// pour optimiser la vitesse d'execution : comparer la distance entre l'instance et son cluster, avec la distance entre
		// l'instance et le cluster le plus proche de son cluster. En fonction du résultat, on pourra se passer de calculer la distance pour
		// les autres clusters
...
void KMClustering::ComputeClustersCentersDistances(const boolean bUseEvaluationCentroids) {
	...
	for (int i = 0; i < nbClusters; i++) {
		...
		for (int j = 0; j < nbClusters; j++) {
			...
			if (i == j) {
				...
				if (i > j) {
					// la distance entre les 2 centres de clusters a deja ete calculée, la reporter
	...
	for (int i = 0; i < nbClusters; i++) {
		...
		// pour chaque cluster, mémoriser le pointeur du cluster qui lui est le plus proche
		for (int j = 0; j < nbClusters; j++) {
...
int KMClustering::ManageEmptyClusters(const boolean continueClustering) {
	...
	for (int i = 0; i < GetClusters()->GetSize(); i++)
	{
		...
		if (c->GetFrequency() == 0)
			// ne pas se baser sur GetCount(), car les clusters clonés ne contiennent pas réellement les instances
			emptyClusters++;
	}
	...
	if (not continueClustering) {
		...
		// etablir la liste complète des instances et de leurs distances
		for (int i = 0; i < kmClusters->GetSize(); i++)
		{
			KMCluster* c = cast(KMCluster*, GetClusters()->GetAt(i));
		...
		// remettre a jour les stats, pour les clusters qui ont été modifiés
		for (int i = 0; i < kmClusters->GetSize(); i++)
		{
					...
					if (initMethod == KMParameters::VariancePartitioning)
						...
						if (initMethod == KMParameters::ClassDecomposition) {
							...
							if (initMethod == KMParameters::Sample) {
								...
								// tirage aléatoire des k centroides de clusters, puis convergence sur un echantillon de la base, puis initialisation des centres a partir des resultats de cette convergence
								...
								if (bOk) {
									...
									// iterations jusqu'à convergence, sur le % retenu de la base, pour déterminer des centroides initiaux :
...
bool KMClustering::UpdateProgressionBar(const longint instancesNumber, const int iterationsDone, const int movements) {
	...
	// ponderation grossière du % de progression
	if (iterationsDone < 2)
		progression /= 10;
...
void KMClustering::ComputeTrainingConfusionMatrix(const KWAttribute* targetAttribute) {
	...
	kwftConfusionMatrix->Initialize(oaTargetAttributeValues.GetSize());
	...
	while (position != NULL) {
		...
		KWObject* currentInstance = (KWObject*)key;
...
void KMClustering::AddInstancesToClusters(const ObjectArray* instances) {
	...
	// affecter des instances de DB a des clusters dont les centroides ont deja eté calculés
	// le readAll() de la base doit avoir deja été fait
	...
	for (int i = 0; i < GetClusters()->GetSize(); i++) {
		KMCluster* c = cast(KMCluster*, GetClusters()->GetAt(i));
		assert(c->GetModelingCentroidValues().GetSize() > 0); // les centroides doivent avoir ete calculés (on ne fait qu'affecter de nouvelles instances a des centres existants)
	...
	// reaffecter les instances aux clusters, en fonction de leurs centroides precedemment calculés
	for (int i = 0; i < instances->GetSize(); i++) {
		KWObject* instance = cast(KWObject*, instances->GetAt(i));
		if (parameters->HasMissingKMeanValue(instance)) {
			clusteringInitializer->IncrementInstancesWithMissingValuesNumber();
...
void KMClustering::FinalizeReplicateComputing(bool recomputeCentroids) {
	...
	// recalculer les stats et centroides, au cas ou quelques instances n'ont pas été reaffectées à leur cluster d'origine
	...
	for (int i = 0; i < GetClusters()->GetSize(); i++) {
		...
		if (recomputeCentroids)
			c->ComputeIterationStatistics(); // maj des centroides et de la somme des distances, pour la norme choisie. Cette methode doit etre exécutée y compris si un cluster est devenu vide
		else {
		...
		// calculer les sommes des distances au centre du cluster (uniquement pour les normes pas encore calculées)
...
void KMClustering::UpdateGlobalDistancesSum() {
	...
	// parcourir les clusters et cumuler la somme des distances des instances par rapport à leurs clusters respectifs
...
void KMClustering::ComputeGlobalClusterStatistics(ObjectArray* instances) {
	...
	// calculer la distance dans les normes pas encore traitées
	if (GetParameters()->GetDistanceType() == KMParameters::L1Norm) {
...
void KMClustering::CopyFrom(const KMClustering* aSource)
{
	...
	// copie des derniers clusters calculés :
	kmClusters->DeleteAll();
	...
	// copie des meilleurs clusters observés :
	kmBestClusters->DeleteAll();
...
void KMClustering::CloneBestClusters() {
	...
	{
		KMCluster* c = cast(KMCluster*, kmClusters->GetAt(i));
		kmBestClusters->Add(c->Clone());	// NB. on ne clone pas les instances elles-mêmes, mais uniquement les centroides et les stats
	}
}
...
KWFrequencyTable* KMClustering::CreateModalitiesFrequenciesByClusters(const ObjectArray* _clusters) {
	...
	modalityFrequencyByCluster->Initialize(_clusters->GetSize());
	...
	// boucle sur les clusters de 1 à k, pour initialiser les valeurs de la table de contingence avec les effectifs des clusters pour chaque modalité cible
	for (int idxCluster = 0; idxCluster < kmClusters->GetSize(); idxCluster++) {
		...
		for (int idxTargetvalue = 0; idxTargetvalue < oaTargetAttributeValues.GetSize(); idxTargetvalue++) { // boucle sur les modalités de la variable cible
...
NumericKeyDictionary* KMClustering::ComputeInstancesToClustersByAscDistance() const {
	...
	NumericKeyDictionary* instancesToClustersByAscDistance = new NumericKeyDictionary; // cle = instance KWObject *, valeur = ObjectArray * contenant la liste de KMCluster *, tries par distance croissante à cette instance
...
void KMClustering::ComputeClusteringLevels(KWClass* kwcModeling, ObjectArray* attributesStats, ObjectArray* clusters) {
	...
	for (int idxCluster = 0; idxCluster < clusters->GetSize(); idxCluster++)
	{
		...
		while (position != NULL) {
			...
			KWObject* instance = (KWObject*)key;
	...
	// Le level est défini par Level = 1 – (Cost/NullCost), sachant que NullCost est calculé a partir d'une table de frequence "nulle"
	...
	// mise a jour pour modalités groupées :
...
void KMClustering::ComputeClusteringLevels(KWDatabase* instances, KWClass* kwcModeling, ObjectArray* attributesStats, ObjectArray* clusters) {
	...
	// a partir des tables de contingence obtenues, calculer les levels de clustering, et les stocker dans une structure pour utilisation
	// lors de l'ecriture du rapport de modelisation
	// Le level est défini par Level = 1 – (Cost/NullCost), sachant que NullCost est calculé a partir d'une table de contingence "nulle"
...
void KMClustering::UpdateClusteringLevelFrequencyTables(const KWObject* kwoObject, const int idCluster) {
	...
	// mise a jour pour modalités groupées (calcul du level de clustering ) :
...
void KMClustering::InitializeClusteringLevelFrequencyTables(const int nbClusters) {
	...
	while (position != NULL) {
		...
		table->Initialize(oaModalities->GetSize());
